...
/**
 * WHOLE only ever reads/writes Robinhood Chain contracts - if the connected
...
export function NetworkGuard() {
  ...
  return (
    <div className="fixed inset-x-0 top-0 z-50 flex flex-wrap items-center justify-center gap-3 border-b border-yellow-500/40 bg-yellow-500 px-4 py-2 text-center text-xs font-medium text-black sm:text-sm">
      ...
      <span>Wrong network — WHOLE only works on Robinhood Chain. Switch to continue.</span>
      <button
        type="button"
        onClick={() => switchChain({ chainId: robinhood.id })}
        disabled={isPending}
        className="rounded-full bg-black px-4 py-1.5 text-xs font-semibold text-white transition hover:opacity-90 disabled:opacity-50"
      >
